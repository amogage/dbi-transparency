...
DWORD SearchForProcessHandles(map<DWORD, map<DWORD, set<string>>>& handles, map<DWORD, string>& procnames)
{
    HMODULE hMod;
...
int main()
{
    ...
    vector<string> blacklist_DBIs = { "coai.exe", "drrun.exe", "flow.exe", "qbdi.exe", "pin.exe", "dynamorio"};
    ...
    DWORD pid = SearchForProcessHandles(handles, procnames);
    for (auto it : blacklist_DBIs)
    {
        if (procnames[pid].find(it) != string::npos)
        {
